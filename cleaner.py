...
class DataCleaner:
    def clean(self, df: pd.DataFrame, banque_nom: str = "Autre banque") -> pd.DataFrame:
        ...
        # Déduplication intelligente pour gérer les overlaps
        ...
        if not years.empty:
            ...
            def fix_date_str(d_str):
                ...
                def replace_bad_year(m):
                    y = int(m.group(0))
                    if y < 2000 or y > 2100: return str(mode_year)
                    return m.group(0)
    ...
    def _parse_amount(self, val) -> Optional[float]:
        ...
        if last_sep_idx != -1:
            ...
            if chars_after_sep == 3: s = s.replace(".", "").replace(",", "")
            elif chars_after_sep in (1, 2): s = s[:last_sep_idx].replace(".", "").replace(",", "") + "." + s[last_sep_idx+1:]
            else: s = s.replace(".", "").replace(",", "")
        try: return float(s)
        except: return None

    def _merge_libelles_minimal(self, df: pd.DataFrame) -> pd.DataFrame:
        if 'Libellé' not in df.columns or df.empty: return df
    ...
    def _remove_fake_balances(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df.reset_index(drop=True)
        to_drop, opening_indices, closing_indices = [], [], []
        for i, row in df.iterrows():
            ...
            if no_mvt or zero_mvt:
                if re.search(r'opening|ouverture|solde\s*initial|solde\s*(au|de)\s*d[ée]but|report|solde\s*ant[ée]rieur|solde\s*pr[ée]c[ée]dent', lib):
                    ...
                    if i < len(df) / 2: opening_indices.append(i)
                    else: closing_indices.append(i)
        if len(opening_indices) > 1: to_drop.extend(opening_indices[1:])
        if len(closing_indices) > 1: to_drop.extend(closing_indices[:-1])
        return df.drop(index=to_drop).reset_index(drop=True)

    def _sort_by_page_line(self, df: pd.DataFrame) -> pd.DataFrame:
        sort_cols = [c for c in ['Page_Num', 'Numero_Ligne'] if c in df.columns]
        if sort_cols: df = df.sort_values(by=sort_cols, kind='mergesort', na_position='first')
        return df.reset_index(drop=True)
    ...
    def _force_balance_positions(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        mid_point = len(df) / 2
        mask_opening = (is_opening | (is_solde_au & (df.index < mid_point))) & ~is_closing
        mask_closing = (is_closing | (is_solde_au & (df.index >= mid_point))) & ~is_opening
        df_opening = df[mask_opening]
        df_closing = df[mask_closing]
        df_trans = df[~(mask_opening | mask_closing)]
        final_parts = []
        if not df_opening.empty: final_parts.append(df_opening)
        final_parts.append(df_trans.sort_index())
        if not df_closing.empty: final_parts.append(df_closing)
    ...
    def _post_process_by_bank(self, df: pd.DataFrame, banque_nom: str) -> pd.DataFrame:
        ...
        if "unics" in banque_lower:
            if 'Libellé' in df.columns: df['Libellé'] = df['Libellé'].apply(lambda x: re.sub(r'\s+', ' ', str(x)).strip() if pd.notna(x) else x)
    ...
    def check_consistency(self, df: pd.DataFrame, tolerance: float = 1.0) -> pd.DataFrame:
        ...
        for i in range(1, len(df)):
            ...
            if abs(ecart) > tolerance:
                ecarts[i] = round(ecart, 2)
                
                # --- Intelligence Mathématique ---
                montant_lu = credit_val if credit_val > 0 else debit_val
    ...
    def get_statistics(self, df: pd.DataFrame, banque_nom: str = "Autre banque") -> dict:
        ...
        if 'Solde' in df.columns:
            solde_col = pd.to_numeric(df['Solde'], errors='coerce').dropna()
            if not solde_col.empty:
                stats['solde_ouverture'] = float(solde_col.iloc[0])
                stats['solde_cloture'] = float(solde_col.iloc[-1])
