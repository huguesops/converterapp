...
<style>
    ...
    .metric-card { background: #ffffff; padding: 1rem 1.2rem; border-radius: 12px; border: 1px solid #E0E0E0; box-shadow: 0 1px 4px rgba(0,0,0,0.06); text-align: center; }
...
def _clear_session_in_browser():
    try: localS.deleteItem(LOCAL_STORAGE_KEY, key="skab_delete_session")
    except Exception: pass
...
if "extraction_done" not in st.session_state:
    st.session_state.update({
        ...
        "uploaded_file_name": None, "browser_restore_done": False, "browser_restore_attempts": 0,
    })

if not st.session_state.extraction_done and not st.session_state.extraction_in_progress and not st.session_state.browser_restore_done:
    if _restore_session_from_browser():
        st.session_state.browser_restore_done = True
        st.rerun()
    else:
        st.session_state.browser_restore_attempts += 1
        if st.session_state.browser_restore_attempts >= 5: st.session_state.browser_restore_done = True
        ...
        if st.session_state.get("bank_detect_sig") != file_sig:
            ...
            if get_openrouter_key():
                with st.spinner("🔍 Détection automatique de la banque..."):
                    try:
                        detector = OpenRouterExtractor(api_key=get_openrouter_key(), mode="vision", banque_nom="Autre banque", verbose_debug=False)
                ...
                else: st.info("Banque non détectée automatiquement — sélectionnez-la ci-dessous.")

    banque_sel = st.selectbox("🏦 Banque émettrice", BANK_LIST, key="banque_widget")
    st.session_state.banque_selectionnee = banque_sel

    method = st.radio("🔍 Méthode d'analyse", ["vision", "hybrid"], help="Vision : analyse d'images | Hybride : texte puis vision si nécessaire")

    if not get_openrouter_key():
        st.warning("⚠️ **Clé API manquante**\nAjoutez `OPENROUTER_API_KEY` dans `.streamlit/secrets.toml`")
...
if not st.session_state.extraction_done and not st.session_state.extraction_in_progress and not uploaded_file:
...
# ====================== EXTRACTION ======================
if uploaded_file and not st.session_state.extraction_done and not st.session_state.show_confirm and not st.session_state.extraction_in_progress:
...
if st.session_state.show_confirm:
    if not get_openrouter_key(): st.error("❌ Clé API OpenRouter manquante.")
    else:
        col1, col2 = st.columns([3, 1])
        with col1: st.info(f"**Analyse imminente** — Banque: **{st.session_state.banque_selectionnee}**")
        with col2:
...
if st.session_state.extraction_in_progress:
    if not get_openrouter_key():
        ...
        try:
            extractor = OpenRouterExtractor(api_key=get_openrouter_key(), mode=st.session_state.extraction_method, banque_nom=st.session_state.banque_selectionnee, verbose_debug=False)
            if st.session_state.extraction_method != "vision":
                ...
                st.progress((current - 1) / total_pages, text=f"🔍 Analyse en cours — pages {current} à {batch_end} sur {total_pages}...")

                batch_transactions = extractor.extract_transactions(st.session_state.pdf_bytes_cache, current, batch_end, total_pages, starting_balance=st.session_state.last_known_balance)
                ...
                st.session_state.failed_pages = sorted(set(st.session_state.failed_pages) | set(extractor.failed_pages))
...
if st.session_state.retry_failed_pages:
    if not get_openrouter_key():
        ...
        try:
            extractor = OpenRouterExtractor(api_key=get_openrouter_key(), mode="vision", banque_nom=st.session_state.banque_selectionnee, verbose_debug=False)
...
if st.session_state.extraction_done and st.session_state.df_clean is not None:
    ...
    if st.session_state.failed_pages:
        ...
        with col_warn: st.error(f"⚠️ {len(st.session_state.failed_pages)} page(s) n'ont pas pu être lues (Pages {pages_str}).")
        with col_btn:
    ...
    with col1:
        ...
        st.markdown(f'<div class="metric-card"><div class="label">Solde ouverture</div><div class="value {"positive" if so and so>=0 else "negative"}">{f"{so:,.0f} FCFA" if so is not None else "N/A"}</div></div>', unsafe_allow_html=True)
    with col2:
        sc = stats.get('solde_cloture')
        st.markdown(f'<div class="metric-card"><div class="label">Solde clôture</div><div class="value {"positive" if sc and sc>=0 else "negative"}">{f"{sc:,.0f} FCFA" if sc is not None else "N/A"}</div></div>', unsafe_allow_html=True)
    with col3:
        net = stats.get('net', 0)
        st.markdown(f'<div class="metric-card"><div class="label">Flux net</div><div class="value {"positive" if net>=0 else "negative"}">{net:,.0f} FCFA</div></div>', unsafe_allow_html=True)

    # --- L'ALERT INTELLIGENTE ---
    ecart_oc = stats.get('ecart_ouverture_cloture')
    if ecart_oc is None:
        st.info("ℹ️ Contrôle indisponible : soldes d'ouverture/clôture non détectés.")
    elif abs(ecart_oc) <= 1:
        st.success("✅ Cohérence parfaite vérifiée : Aucun écart mathématique.")
    else:
        st.error(f"🚨 Écart mathématique détecté de {ecart_oc:,.0f} FCFA. L'intelligence d'audit a diagnostiqué les lignes en cause ci-dessous.")
    
    col4, col5, col6 = st.columns(3)
    with col4: st.markdown(f'<div class="metric-card"><div class="label">Total crédits</div><div class="value credit">{stats.get("total_credit", 0):,.0f} FCFA</div></div>', unsafe_allow_html=True)
    with col5: st.markdown(f'<div class="metric-card"><div class="label">Total débits</div><div class="value debit">{stats.get("total_debit", 0):,.0f} FCFA</div></div>', unsafe_allow_html=True)
    with col6: st.markdown(f'<div class="metric-card"><div class="label">Lignes extraites</div><div class="value balance">{len(df_display)}</div></div>', unsafe_allow_html=True)
    ...
    fig.update_layout(hovermode="x unified", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), margin=dict(l=40, r=20, t=40, b=40))
    ...
    with col_d1: date_debut = st.date_input("Du", value=date_min, min_value=date_min, max_value=date_max)
    with col_d2: date_fin = st.date_input("Au", value=date_max, min_value=date_min, max_value=date_max)
    ...
    with col_xlsx:
        ...
        sheet1_cols = ['Date', 'Référence', 'Libellé', 'Débit', 'Crédit', 'Solde', 'Écart', 'Diagnostic']
        ...
        is_solde_row = sheet1_df['Libellé'].astype(str).str.lower().str.contains(solde_pattern, na=False, regex=True) if 'Libellé' in sheet1_df.columns else pd.Series(False, index=sheet1_df.index)
        ...
        if len(sheet1_df) and len(solde_series) and pd.notna(solde_series.iloc[0]):
            opening_balance = float(solde_series.iloc[0]) if is_solde_row.iloc[0] else float(solde_series.iloc[0]) - (0.0 if pd.isna(montant_series.iloc[0]) else float(montant_series.iloc[0]))

        wb = Workbook()
        ws = wb.active
        ws.title = "RELEVE"

        # Entêtes avec le nouveau Diagnostic Intelligent
        headers = ["Date", "Référence", "Libellé", "Montant", "Solde courant", "Écart", "Diagnostic IA"]
        ...
        # Stylisation spéciale pour les erreurs de diagnostic
        ...
        for i in range(len(sheet1_df)):
            ...
            ecart_val = sheet1_df.iloc[i].get('Écart')
            diag_val = sheet1_df.iloc[i].get('Diagnostic')

            c_date = ws.cell(row=row_ptr, column=1, value=date_val)
            c_date.number_format = 'dd/mm/yyyy'
            c_date.border = thin_border

            ws.cell(row=row_ptr, column=2, value=ref_val).border = thin_border
            ws.cell(row=row_ptr, column=3, value=libelle).border = thin_border
            ...
            if is_solde and pd.notna(solde_series.iloc[i]): c_solde.value = float(solde_series.iloc[i])
            elif row_ptr == 2: c_solde.value = f"={opening_balance:.0f}+D{row_ptr}"
            ...
            c_diag.border = thin_border
            
            if pd.notna(ecart_val):
                c_ecart.number_format = '#,##0;-#,##0'
                c_ecart.font = red_font
                c_ecart.fill = red_fill
                c_diag.font = red_font
                c_diag.fill = red_fill
